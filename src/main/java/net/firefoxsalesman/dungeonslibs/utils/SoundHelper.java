...
public class SoundHelper {
	...
	// this gets multiplied by 16.0F, so the volume indicates from how many chunks
	// away the sound can be heard
	...
	public static void playLightningStrikeSounds(Entity soundEmissionTarget) {
		...
		soundEmissionTarget.level().playSound(player,
				...
				SoundEvents.LIGHTNING_BOLT_THUNDER, soundEmissionTarget.getSoundSource(), volumeLimit,
				0.8F + RNG.nextFloat() * 0.2F);
		soundEmissionTarget.level().playSound(player,
				soundEmissionTarget.getX(), soundEmissionTarget.getY(), soundEmissionTarget.getZ(),
				SoundEvents.LIGHTNING_BOLT_IMPACT, soundEmissionTarget.getSoundSource(), volumeLimit,
				0.5F + RNG.nextFloat() * 0.2F);
	}
	...
	public static void playAttackSweepSound(Entity soundEmissionTarget) {
		soundEmissionTarget.level().playSound(getPlayerFrom(soundEmissionTarget),
				soundEmissionTarget.blockPosition(),
				SoundEvents.PLAYER_ATTACK_SWEEP, soundEmissionTarget.getSoundSource(), volumeLimit,
				standardPitch);
	}
	...
	public static void playBellSound(Entity soundEmissionTarget) {
		...
		soundEmissionTarget.level().playSound(player,
				soundEmissionTarget.blockPosition(),
				SoundEvents.BELL_BLOCK, soundEmissionTarget.getSoundSource(), volumeLimit,
				standardPitch);
		soundEmissionTarget.level().playSound(player,
				soundEmissionTarget.blockPosition(),
				SoundEvents.BELL_RESONATE, soundEmissionTarget.getSoundSource(), volumeLimit,
				standardPitch);
	}

	public static void playCreatureSound(Entity soundEmissionTarget, SoundEvent soundEvent) {
		soundEmissionTarget.level().playSound(getPlayerFrom(soundEmissionTarget),
				soundEmissionTarget.blockPosition(), soundEvent, soundEmissionTarget.getSoundSource(),
				volumeLimit, standardPitch);
	...
	public static void playBeaconSound(Entity soundEmissionTarget, boolean activate) {
		if (activate) {
			soundEmissionTarget.level().playSound(getPlayerFrom(soundEmissionTarget),
					soundEmissionTarget.blockPosition(), SoundEvents.BEACON_ACTIVATE,
					soundEmissionTarget.getSoundSource(), volumeLimit, standardPitch);

		} else {
			soundEmissionTarget.level().playSound(getPlayerFrom(soundEmissionTarget),
					soundEmissionTarget.blockPosition(), SoundEvents.BEACON_DEACTIVATE,
					soundEmissionTarget.getSoundSource(), volumeLimit, standardPitch);
	...
	public static void playKnockbackSound(Entity soundEmissionTarget) {
		soundEmissionTarget.level().playSound(getPlayerFrom(soundEmissionTarget),
				soundEmissionTarget.blockPosition(), SoundEvents.PLAYER_ATTACK_KNOCKBACK,
				soundEmissionTarget.getSoundSource(), volumeLimit, standardPitch);
	}
