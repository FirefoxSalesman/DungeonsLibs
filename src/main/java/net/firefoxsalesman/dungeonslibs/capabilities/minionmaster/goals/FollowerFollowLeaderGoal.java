...
public class FollowerFollowLeaderGoal extends Goal {
	...
	public FollowerFollowLeaderGoal(Mob mobEntity, double followSpeed, float maxDist, float minDist,
			boolean passesThroughLeaves) {
		 ...
		 * throw new
		 * IllegalArgumentException("Unsupported mob type for FollowOwnerGoal");
		 * }
		 */
	}
	...
	private void teleportToOwner() {
		...
		for (int i = 0; i < 10; ++i) {
			...
			boolean flag = this.maybeTeleportTo(blockpos.getX() + j, blockpos.getY() + k,
					blockpos.getZ() + l);
			if (flag) {
				return;
			}
	...
	private boolean maybeTeleportTo(int p_226328_1_, int p_226328_2_, int p_226328_3_) {
		if (Math.abs((double) p_226328_1_ - this.owner.getX()) < 2.0D
				&& Math.abs((double) p_226328_3_ - this.owner.getZ()) < 2.0D) {
			return false;
		} else if (!this.canTeleportTo(new BlockPos(p_226328_1_, p_226328_2_, p_226328_3_))) {
			return false;
		} else {
			this.mobEntity.moveTo((double) p_226328_1_ + 0.5D, p_226328_2_, (double) p_226328_3_ + 0.5D,
					this.mobEntity.getYRot(), this.mobEntity.getXRot());
			this.navigator.stop();
			return true;
		}
	}
	...
	private boolean canTeleportTo(BlockPos p_226329_1_) {
		...
		if (pathnodetype != BlockPathTypes.WALKABLE) {
			...
			if (!this.passesThroughLeaves && blockstate.getBlock() instanceof LeavesBlock) {
				...
				return this.world.noCollision(this.mobEntity,
						this.mobEntity.getBoundingBox().move(blockpos));
			}
		}
	}
