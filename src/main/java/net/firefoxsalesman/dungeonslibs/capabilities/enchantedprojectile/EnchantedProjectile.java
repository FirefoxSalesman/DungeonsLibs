...
public class EnchantedProjectile {
	...
	public void setEnchantments(ItemStack itemStack) {
		...
		Map<Enchantment, Integer> builtInEnchantments = cap.getAllBuiltInEnchantmentInstances().stream()
				.collect(Collectors.groupingBy(enchantmentData -> enchantmentData.enchantment,
						Collectors.summingInt(value -> value.level)));
		builtInEnchantments.forEach((enchantment, integer) -> itemStackEnchantmentInstance.compute(enchantment,
				(enchantment1, integer1) -> integer1 == null ? integer : integer1 + integer));
	...
	public CompoundTag serializeNBT() {
		...
		enchantmentDataList.forEach(enchantmentData -> {
			...
			enchantmentDataNBT.putString("id", String
					.valueOf(ForgeRegistries.ENCHANTMENTS.getKey(enchantmentData.enchantment)));
	...
	public void deserializeNBT(CompoundTag tag) {
		Map<Enchantment, Integer> enchantmentIntegerMap = EnchantmentHelper
				.deserializeEnchantments(tag.getList(ENCHANTMENT_DATA_KEY, 10));
		enchantmentDataList = enchantmentIntegerMap.entrySet().stream()
				.map(entry -> new EnchantmentInstance(entry.getKey(), entry.getValue()))
				.collect(Collectors.toList());
	}
