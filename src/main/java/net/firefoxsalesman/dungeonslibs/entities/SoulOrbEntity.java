...
public class SoulOrbEntity extends Entity implements IEntityAdditionalSpawnData {
	...
	public SoulOrbEntity(Player followingPlayer, Level level, double x, double y, double z, float value) {
		this(LibEntityTypes.SOUL_ORB.get(), level);
	...
	public void tick() {
		...
		if (!this.level().isClientSide
				&& (this.followingPlayer == null || this.followingPlayer.isSpectator())) {
			this.discard();
		}

		if (this.followingPlayer != null && this.floatTime == 0) {
			Vec3 vector3d = new Vec3(this.followingPlayer.getX() - this.getX(), this.followingPlayer.getY()
					+ (double) this.followingPlayer.getEyeHeight() / 2.0D - this.getY(),
	...
	private void setUnderwaterMovement() {
		...
		this.setDeltaMovement(vector3d.x * (double) 0.99F, Math.min(vector3d.y + (double) 5.0E-4F, 0.06F),
				vector3d.z * (double) 0.99F);
	}
