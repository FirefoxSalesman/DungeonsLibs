...
public abstract class CrossbowItemMixin {
	...
	private static void handlePerformShooting(Level level, LivingEntity shooter, InteractionHand useHand,
			ItemStack crossbow, float velocity, float inaccuracy, CallbackInfo ci) {
		ci.cancel();
		if (!LOGGED_WARNING) {
			DungeonsLibs.LOGGER.debug(
					"CrossbowItem#performShooting was canceled by Dungeons Libraries and replaced with its CrossbowGear#fireCrossbowProjectiles");
			LOGGED_WARNING = true;
		}
	...
	private float libraries_use_getShootingPower(ItemStack stack, Level level, Player player,
	...
	private static void getChargeDuration(ItemStack stack, CallbackInfoReturnable<Integer> cir) {
		cir.setReturnValue((int) RangedAttackHelper.getVanillaCrossbowChargeTime(null, stack)); // TODO: Should
													// take in a
													// LivingEntity
													// to be able to
													// check for
	...
	private static void handleGetArrow(Level level, LivingEntity shooter, ItemStack weapon, ItemStack ammo,
